    ...
    try {
      ...
      for (const item of products) {
        ...
        for (const batch of product.batches) {
          if (batch.qty <= 0 || new Date(batch.expirationDate) < new Date()) continue;
      ...
      for (const item of products) {
        ...
        if (product.batches && product.batches.length > 0) {
          ...
          for (const batch of product.batches) {
            ...
            if (batch.qty <= 0 || isExpired) continue;  
...
router.post("/orders/addOrder", protect, async (req, res) => {
    try {
      ...
      for (const item of products) {
        ...
        // Sort embedded batches by expirationDate (ascending)
        const validBatches = product.batches
          .filter(batch => batch.expirationDate > today && batch.qty > 0)
        ...
        for (const batch of validBatches) {
          ...
          const usedQty = Math.min(remainingQty, batch.qty);
          batch.qty -= usedQty;
          remainingQty -= usedQty;
        ...
        if (remainingQty > 0) {
          return res.status(400).json({ error: `Not enough valid batches for product: ${product.productName}` });
        }
  
        const oldStock = product.qty;
  
        // Update product global quantity and itemsSold
        product.qty -= item.quantity;
        product.itemsSold += item.quantity;
        ...
        // Save the updated product (with batches modified)
        await product.save();
  
        // Run stock notification logic
        await checkStockAndNotify(product, oldStock);
  
        await product.updateStockStatus();
  
        // Optionally: attach used batch info to product item in order
      ...
      const newOrder = new Order({
        userId,
        shipping,
        products, // includes batchesUsed if you keep it
        totalAmount,
        amountPaid,
        rest,
        paymentStatus,
